...
- [x] Edge case: a subfolder present in the saved fold state no longer exists on disk (renamed/deleted since last visit) — stale entries are simply unused, no active cleanup needed — not exercised live (would require renaming a real folder mid-test); verified by code reading: `isModuleFolderExpanded` is a plain `Set.has()` check against a path string, which just returns `false` for a path that no longer resolves to anything rendered — nothing dereferences the stale entry itself
